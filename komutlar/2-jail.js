...
if(!message.member.roles.cache.has('712416540305391698') && !message.member.hasPermission("ADMINISTRATOR")) { //jail yetkilisinin rol id
...
if(kişi.roles.cache.has('712416540305391698') && !message.member.hasPermission("ADMINISTRATOR")) { //jail yetkilisinin rol id
const embed = new Discord.MessageEmbed()
.setColor('RED')
...
kişi.roles.add('693950335030394881'); //JAİL ROLÜ İD
kişi.roles.cache.forEach(r => {
kişi.roles.remove(r.id)
db.set(`${message.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )
})  
client.channels.cache.get('668389364291862528').send(hapis) //Jaile Girince Log Mesajı Hangi Kanala Gidicekse O Kanalın İDSİ.
kişi.send(dm) 
message.channel.send(embed2)
setTimeout(async () =>{
kişi.roles.remove('693950335030394881') //Alınacak ROL İD
client.channels.cache.get('668389364291862528').send(tahliye) //Jailden Çıkınca Log Mesajı Hangi Kanala Gidicekse O Kanalın İDSİ.
}, ms(zaman));
setTimeout(async () =>{
